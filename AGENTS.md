...
2. **No data fabrication.** Functions must not generate fake sensor data or return placeholder values. If data is unavailable, raise an exception or return None.
   ...
   **External databases must be downloaded from their official source — never assumed, synthesized, or substituted.**
   Each domain declares its required data sources in `config/domains/{domain}.yaml → data_sources`. The pipeline enforces this at COMPUTE C1 gate (`tools/fetch_domain_data.py --verify`). Code must block with download instructions if data is absent, not silently proceed.

   | Domain | Required database | Official source | Local path | Download tool |
   |--------|------------------|-----------------|------------|---------------|
   | `structural` | PEER NGA-West2 ground motions (.AT2) | https://ngawest2.berkeley.edu (free account required) | `db/excitation/records/` | `tools/peer_downloader.py` |
   | `structural` | LANL / Z24 / IASC-ASCE SHM benchmarks | Published datasets (see `structural.yaml → data_sources.benchmarks`) | `db/benchmarks/` | `tools/fetch_benchmark.py` |
   | `water` | ERA5 reanalysis / field measurements | https://cds.climate.copernicus.eu | `db/field/` | `tools/fetch_domain_data.py --domain water` |
   | `air` | Wind field data / CFD validation sets | Published CFD benchmarks | `db/field/` | `tools/fetch_domain_data.py --domain air` |
   | `biomedical` | PhysioNet clinical records | https://physionet.org (credentialed access may apply) | `db/field/` | `tools/fetch_domain_data.py --domain biomedical` |
   | `economics` | FRED macroeconomic series | https://fred.stlouisfed.org (free API key) | `db/field/` | `tools/fetch_domain_data.py --domain economics` |

   **Synthetic data** (generated by `tools/generate_degradation.py`) is allowed ONLY as a **complement** to real data for Conference and Q4 papers. For Q3/Q2/Q1, synthetic data alone is rejected — real field or benchmark data is mandatory.

   If a required database is not yet downloaded, the code must:
   1. Print the official URL and download instructions
   2. Exit with a non-zero code (gate failure)
   3. Never fall back to a different dataset or invent values
